...
徽章铸造系统使用 6 个训练核心字段驱动空间填充曲线可视化。

### 六大核心字段

| 字段 | 来源 | 说明 |
|------|------|------|
| 时间戳 | 训练日期 | ISO 8601 格式 |
| 训练时长 | session.duration_minutes | 单位：分钟 |
| 总拍数 | session.total_shots | 该场次所有击球总数 |
| 回球速度均值 | 所有 player 回球 speed_kmh 的平均值 | 单位：km/h |
| 轨迹顶点高度均值 | 所有 player 回球 trajectory.apex_height_m 的平均值 | 单位：米 |
| 心率峰值 | heart_rate 中的最大值 | 单位：bpm |
...
| 条件 | 曲线类型 | 说明 |
|------|---------|------|
| < 80 km/h | 2D Hilbert → Gosper | 从 Hilbert 曲线生长，过渡到 Gosper 六边形曲线 |
| ≥ 80 km/h | 3D Z-Order | Morton 曲线三维投影，多层叠加 |

#### 2. 渐变颜色（时间戳）

- 从时间戳的毫秒值派生色相 `hue`，映射到绿青色域 `[110°, 180°]`
- 渐变起点：`hsl(hue, 70%, 85%)` — 亮色
- 渐变终点：`hsl(hue+20°, 75%, 35%)` — 暗色
- 反转色：`hsl((hue+180°) mod 360°, 80%, 50%)` — 互补色

#### 3. 变点/重叠度（训练时长）

**2D 曲线：**
- `variation = clamp(duration_min / 120, 0, 1) * 0.7 + 0.3`
- 控制 Hilbert→Gosper 的过渡位置：`transition = clamp(0.4 + variation * 0.3, 0.2, 0.85)`
- 训练越久，Hilbert 部分越长，Gosper 部分越短
...
- `stroke_width = 1.5 + clamp(total_shots / 2000, 0, 1) * 3.5`
- 范围：`[1.5, 5.0]`
- 拍数越多，线条越粗

#### 5. 不透明度（轨迹顶点高度均值）

- `opacity = 0.9 + clamp(avg_apex / 3.0, 0, 1) * 0.1`
- 范围：`[0.9, 1.0]`
- 轨迹越高，越接近完全不透明

#### 6. 反转色位置（心率峰值）

- `inverted_pos = clamp(peak_hr / 200, 0, 1)`
- 在曲线的 `inverted_pos` 位置处（归一化），使用反转色（互补色）绘制一小段高亮
- 心率越高，反转色出现的位置越靠后
